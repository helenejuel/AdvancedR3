...
```{r run-wrangle-functions, eval = FALSE}
lipidomics %>%
...
```{r reg-update-roles-function, eval = FALSE}
recipe(lipidomics_wide) %>%
  update_role(metabolite_lipid_ch_3_1, age, gender,
    new_role = "predictor"
  ) %>%
...
```{r run-recipe-function, eval = FALSE}
...
```{r run-workflow-function, eval = FALSE}
...
```{r simple-model-output, eval = FALSE}
fitted_model %>%
  extract_fit_parsnip() %>%
  broom::tidy(exponentiate = TRUE) # changes logOR to OR
```

```{r simple-model-function, eval = FALSE}
fitted_model %>%
  tidy_model_output()
```

```{r putting-it-together, eval = FALSE}
...
```{r split-function, eval = FALSE}
lipidomics %>%
  split_by_metabolite()
```

```{r split-model-function, eval=FALSE}
...
```{r model-estimates-targets}
model_estimates <- tar_read(df_model_estimates)

```

```{r plot-estimates, eval = FALSE}
...
```{r plot-estimates-function, eval=FALSE}
model_estimates %>% 
    plot_estimates()

```

```{r plot-estimates-targets}
